    ...
    <header className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-white border-b border-slate-200 shadow-sm py-2' : 'bg-white/95 backdrop-blur-md border-b border-slate-100 py-3'}`}>
      <div className="max-w-[1440px] mx-auto px-4 sm:px-6">
        <div className="flex items-center gap-4">
          {/* Logo */}
          <Link href="/" className="flex flex-col justify-center group shrink-0 mr-2">
            ...
            {navItems.map((item) => (
              ...
              >
                {item.href ? (
                  <Link 
                    href={item.href}
                    className={`px-2.5 py-1.5 text-[13px] font-semibold transition-colors duration-150 rounded-lg hover:bg-slate-100 hover:text-electric flex items-center whitespace-nowrap ${pathname === item.href ? 'text-electric bg-slate-50' : 'text-slate-700'}`}
                  >
                    {item.label}
                  </Link>
                ) : (
                  <button
                    className={`px-2.5 py-1.5 text-[13px] font-semibold transition-colors duration-150 rounded-lg hover:bg-slate-100 hover:text-electric flex items-center gap-1 whitespace-nowrap ${activeDropdown === item.label ? 'text-electric bg-slate-50' : 'text-slate-700'}`}
                  >
                    {item.label}
          ...
          <div className="hidden xl:flex items-center ml-2 shrink-0">
            <Link
              ...
              className="bg-blue-50 text-blue-600 border border-blue-200 text-[11px] font-bold px-3.5 py-1.5 rounded-full hover:bg-blue-100 transition-colors flex items-center gap-1.5 shrink-0"
            >
            ...
            <Link
              href="/contact"
              className="bg-electric text-white text-[12px] font-bold px-4 py-2 rounded-full hover:bg-electric/90 transition-colors shadow-sm hover:shadow-md flex items-center gap-1.5 group whitespace-nowrap"
            >
              Contact Us
          ...
          <div className="xl:hidden flex items-center gap-2 ml-auto">
            <Link
              href="/contact"
              className="bg-electric text-white text-[11px] font-bold px-3 py-1.5 rounded-full hover:bg-electric/90 transition-colors whitespace-nowrap"
            >
              Contact
            </Link>
            <button
